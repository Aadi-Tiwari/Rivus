  ...
  constructor(scale: number = 10.0) {
    ...
    super({
      ...
      void main() {
        ...
        // Directional traveling swells. Water moves mostly vertically as the wave
        // passes through it, so lateral drift is damped hard and the surface is
        // driven by superposed sine trains instead. Integer time multipliers keep
        // every train commensurate with uLoopPeriod, so the whole field still loops.
        // Long, slow swells. Wavenumbers and speeds are kept low so the surface reads
        // as a calm background rather than competing with the data on top of it.
        float swell = 0.0;
        swell += sin(originalPos.x *  0.42 + originalPos.z *  0.23 + continuousTime * 1.0) * 0.44;
        swell += sin(originalPos.x * -0.21 + originalPos.z *  0.60 + continuousTime * 2.0) * 0.26;
        swell += sin(originalPos.x *  0.90 + originalPos.z * -0.50 + continuousTime * 3.0) * 0.12;

        // Apply distortion to original position
        vec3 distortion = vec3(
          displacementX * 0.18,
          displacementY * 0.55 + swell,
          displacementZ * 0.18
        ) * uNoiseIntensity;
