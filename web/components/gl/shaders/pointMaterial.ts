  ...
  constructor() {
    super({
      ...
      void main() { 
        ...
        // Near-field particles are sparse in screen space because the grid is fixed
        // in world space, so let them grow larger to keep the foreground covered.
        gl_PointSize = clamp(vDistance * uBlur * uPointSize, 4.0, 18.0);
      }`,
      fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        // Height must never drive opacity to zero, or a passing trough erases that
        // region of the surface and the hero shows through as flat black. Floor it
        // so troughs read as dim deep water that is still lit.
        float heightFade = mix(0.62, 1.0, smoothstep(-0.75, 0.35, vPosY));

        // Widen the focal band. At full strength only a ~1 unit deep slice stays
        // lit, which leaves most of the plane invisible once it is all revealed.
        // Clamp the ceiling well below 1.0 so out-of-focus foreground water keeps a
        // floor of visible alpha. Without it the near edge of the plane, which is
        // the bottom of the frame, fades to nothing and the screen looks half empty.
        float dofFade = 1.04 - clamp(vDistance * 0.30, 0.0, 0.66);

        float alpha = dofFade * heightFade * uOpacity * revealMask * uRevealProgress * sparkleBrightness;

        // Water colour by wave height: troughs read as deep water, crests break to foam.
        // Alpha already fades troughs out, so the ramp must not darken them again
        // or the two effects multiply and the surface goes black.
        float h = smoothstep(-0.45, 0.55, vPosY);
        // Deep ocean through steel blue to foam. The field only renders on the marketing
        // pages now, never behind the posterior heatmap, so it can carry real water hue
        // without competing with the data colours.
        vec3 deep = vec3(0.03, 0.10, 0.20);
        vec3 mid  = vec3(0.16, 0.48, 0.68);
        vec3 foam = vec3(0.82, 0.94, 1.00);
        vec3 waterColor = mix(deep, mid, smoothstep(0.0, 0.50, h));
        waterColor = mix(waterColor, foam, smoothstep(0.52, 0.90, h));

        // Sparkle is specular glint off the surface, so it pushes toward white
        // rather than saturating the blue.
        waterColor = mix(waterColor, foam, clamp((sparkleBrightness - 1.35) * 0.9, 0.0, 1.0));

        gl_FragColor = vec4(waterColor, mix(alpha, sparkleBrightness - 1.1, uTransition));
      }`,
      uniforms: {
